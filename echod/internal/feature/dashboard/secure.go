...
// NNpsk0 handshake, the same family this device's link to Home Assistant uses. Both ends mix the
// key into the handshake, so the wrong key simply fails it; the key itself never crosses the network,
// and everything after the handshake - the hello, the pictures, the touches - is encrypted and
// authenticated.
//
...
// any size is as many records as it takes. The server's side is dashcast/secure.go, which has to
// match this one.
...
// clientHandshake is this device's side of the handshake. It fails when the server's key is not this
// device's.
