...
func (c *Client) Boards(ctx context.Context) ([]Board, error) {
	...
	if raw, err := s.call(map[string]any{"type": "get_panels"}); err == nil {
		...
		if json.Unmarshal(raw, &panels) == nil {
			// Webpage dashboards are left out: dashcast does not show them, since they are often a
			// tool (a code editor, the ESPHome dashboard) that anyone with its key could then work.
			var names []string
